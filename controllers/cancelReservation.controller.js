...
const canceleservationPost = async (req, res) => {
    try {
        const data = req.body;
        let userEmail;
        ...
        let updatesForReservation = {
            // Set update cho reservation
            status: 2
        }

        await firestore.collection('client').doc(req.cookies.userId).update(updates);
        await firestore.collection('reservation').doc(client.checkReservation).update(updatesForReservation);
